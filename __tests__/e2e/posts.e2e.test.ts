...
describe(RouterPaths.posts, () => {
    ...
    it(`should return 200 and array of posts after get/ request`, async () => {
        ...
        correctBodyPost.blogId = testBlog.body.id
        const testPost = await postTestRepository.create(correctBodyPost)
        const testPost1 = await postTestRepository.create(correctBodyPost)
        const testPost2 = await postTestRepository.create(correctBodyPost)
    ...
    it('should update existing post with correct data', async () => {
        ...
        correctBodyPost.blogId = testBlog.body.id
        const testPost = await postTestRepository.create(correctBodyPost)
        updatedCorrectBodyPost.blogId = correctBodyPost.blogId
        await postTestRepository.update(testPost.body.id, updatedCorrectBodyPost)
